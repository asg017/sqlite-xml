...
class TestXml(unittest.TestCase):
  ...
  def test_xml_valid(self):
    self.skipTest("asdf")
    xml_valid = lambda pattern: db.execute("select xml_valid(?)", [pattern]).fetchone()[0]
    self.assertEqual(
      xml_valid("[0-9]{3}-[0-9]{3}-[0-9]{4}"),
      1
    )
    self.assertEqual(
      xml_valid("no("),
  ...
  def test_xml_extract(self):
    self.skipTest('asdf')
    xml_extract = lambda document, xpath: db.execute("select xml_extract(?, ?)", [document, xpath]).fetchone()[0]
    self.assertEqual(
      xml_extract(""),
      1
    )
  ...
  def test_xml_each(self):
    xml_each = lambda document, xpath: execute_all("select rowid, * from xml_each(?, ?)", [document, xpath])
    self.assertEqual(
      xml_each("""
        <items>
      ...
      """, '//items/item'),
      [
        {'rowid': 0, 'xml': '<item><element>yo!</element></item>', 'text': 'yo!'},
